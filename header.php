...
<head>
    ...
    <?php wp_head(); ?>
    <?php
    // Custom Header Code (仅管理员可设置，静态化后安全)
    if ( ! empty( $options['code_head'] ) ) {
        echo wp_kses_post( $options['code_head'] ) . "\n";
    }

    // Custom CSS (仅管理员可设置，静态化后安全)
    if ( ! empty( $options['code_css'] ) ) {
        echo '<style>' . wp_strip_all_tags( $options['code_css'] ) . '</style>' . "\n";
    }
    ?>
</head>
<body <?php body_class( 'bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-200 font-sans antialiased flex flex-col min-h-screen' ); ?>>
<?php wp_body_open(); ?>
